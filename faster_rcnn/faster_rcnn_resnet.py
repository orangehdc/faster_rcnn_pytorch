...
from vgg16 import VGG16  #resnet
...
class RPN(nn.Module):
    ...
    def __init__(self):
        ...
        #self.features = VGG16(bn=False) # resnet
        self.resnet = res.resnet(pretrained = False, depth = 101)
        self.features = None
    ...
    def _init_modules(self, fixed_blocks = 2):
        ...
        for p in self.resnet.fc.parameters(): p.requires_grad=False
    ...
    @property
    def loss(self):
        return self.cross_entropy  + self.loss_box * 10  # for classification correctly
...
class FasterRCNN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False):
        ...
        self.rpn = RPN()
        self.rpn._init_modules(fixed_blocks = self.fixed_blocks)  # should be same as self.fixed blocks
        ...
        self.debug = debug
    def fc6fc7(self, x):
        return self.rpn.resnet.layer4(x).mean(3).mean(2)

    def load_pretrained_cnn(self, state_dict):
        self.rpn.resnet.load_state_dict(state_dict)
    ...
    def train(self, mode=True):
        ...
        if mode:
          # Set fixed blocks to be in eval mode
            self.rpn.resnet.eval()
            if self.fixed_blocks == 0:
                self.rpn.resnet.layer1.train()
            if self.fixed_blocks <= 1:
                self.rpn.resnet.layer2.train()

            self.rpn.resnet.layer3.train()
            self.rpn.resnet.layer4.train()
        ...
        self.rpn.resnet.apply(set_bn_eval)
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        ...
        #x = pooled_features.view(pooled_features.size()[0], -1)  # for vgg
        x = pooled_features  # for resnet
        ...
        #x = F.dropout(x, training=self.training)
        x = self.fc6fc7(x)
        #print("fc6fc7 features",x.size())  # batch * 2048
